...
class Afk(commands.Cog):
    ...
    async def afk_get_mention(self, message):
        ...
        async for msg in messages:
            if mention in msg.content:
                ...
                await message.reply(embed=discord.Embed(title=f"{user.name}はAFKです。", description=f"理由：{str(reason)}").set_footer(text="このメッセージは5秒後に削除されます。"), delete_after=5)
    ...
    async def on_message(self, message):
        ...
        if back is None:
            ...
            print(ba)
            if ba == "ok":
                await message.reply("AFKを解除しました")
            else:
